// Find the real chain walker addresses in our run
...
console.log(`cs=1569 base = 0x${cs1569_base.toString(16)}`);

// Dump the walker entry region at cs=1569:0x0ba4..0x0c60 to verify offsets
console.log('\nWalker dump 0x0ba4..0x0c60:');
for (let off = 0xba4; off < 0xc60; off += 16) {
  const bytes = [];
  for (let j = 0; j < 16; j++) bytes.push(mem.readU8(cs1569_base + off + j).toString(16).padStart(2, '0'));
  console.log(`  cs:0x${off.toString(16)} (linear 0x${(cs1569_base + off).toString(16)}): ${bytes.join(' ')}`);
}
