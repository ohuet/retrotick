...
import { VGA_FONT_8X8_ROM, ROM_FONT_8X8_SEG, ROM_FONT_8X8_OFF, ROM_FONT_8X8_ADDR, ROM_FONT_CGA_OFF } from './vga-font-data';
import { VGA_FONT_8X16_ROM, ROM_FONT_8X16_SEG, ROM_FONT_8X16_OFF, ROM_FONT_8X16_ADDR } from './vga-font-16';
...
function setVideoMode(cpu: CPU, emu: Emulator, modeNum: number): void {
  ...
  // Update INT 43h to point to the appropriate font for the new mode
  if (vgaMode.charHeight <= 8) {
...
export function handleInt10(cpu: CPU, emu: Emulator): boolean {
  ...
  switch (ah) {
    ...
    case 0x11: { // Character generator
      if (al === 0x11 || al === 0x14) {
        // AL=11h: Load ROM mono 8x14 font (we use 8x16 as fallback)
        // AL=14h: Load ROM 8x16 font
        // Switch back to 80x25 with 8x16 font
        emu.screenRows = 25;
        emu.charHeight = 16;
        emu.initConsoleBuffer();
        ...
        if (bh === 0) {
          ...
          // ROM 8x16 font (BH=5: 9x14 alt, BH=6: 8x16 — we return 8x16)
          fontSeg = ROM_FONT_8X16_SEG;
          fontOff = ROM_FONT_8X16_OFF;
        } else if (bh === 7) {
          // ROM 8x16 alt font — return 8x16
          fontSeg = ROM_FONT_8X16_SEG;
          fontOff = ROM_FONT_8X16_OFF;
        }
        // BH=2 (8x14): return 8x8 as fallback (no 8x14 font)
        cpu.es = fontSeg;
