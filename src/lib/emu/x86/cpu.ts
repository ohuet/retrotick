...
export class CPU {
  ...
  set ds(val: number) { this._dsVal = val; if (!this.emu?.ne) this.segBases.delete(val); }
  get es(): number { return this._esVal; }
  set es(val: number) { this._esVal = val; if (!this.emu?.ne) this.segBases.delete(val); }
  get fs(): number { return this._fsVal; }
  set fs(val: number) { this._fsVal = val; if (!this.emu?.ne) this.segBases.delete(val); }
  get gs(): number { return this._gsVal; }
  set gs(val: number) { this._gsVal = val; if (!this.emu?.ne) this.segBases.delete(val); }
  ...
  set ss(val: number) {
    this._ssVal = val;
    if (!this.emu?.ne) this.segBases.delete(val); // invalidate cached base for the new SS (skip Win16 NE: segBases is the source of truth)
    this._recomputeSsB32();
  }
  ...
  loadCS(selector: number): void {
    ...
    // Skip for Win16 NE only: the segBases map IS the source of truth there,
    // so deleting it would lose the bind (notably the thunk selector 0xFE →
    // linear 0xF0000, which is not sel*0x10000).
    if (!this.emu?.ne) this.segBases.delete(selector);
    this.cs = selector;
    if (!this.realMode) {
